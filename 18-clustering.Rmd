...
metadatae(pollen)$sc3$k_estimation
```
...
colData(pollen)$pcaReduce <- as.character(pca.red[,32 - 2])
plotPCA(pollen, colour_by = "pcaReduce")
```
...
__Our solution__:
```{r, echo=FALSE}
colData(pollen)$tSNE_kmeans <- as.character(kmeans(pollen@reducedDims$TSNE, centers = 11)$clust)
...
__Our solution__:
```{r, echo=FALSE}
colData(pollen)$SINCERA <- as.character(cutree(hc, k = i))
