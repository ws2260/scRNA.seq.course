...
```{r, out.width = '80%', fig.cap="scmap can be used to compare the annotations of two different samples by providing a one to one mapping between the cells."}
knitr::include_graphics("figures/scmap1.png")
```

```{r, out.width = '80%', fig.cap=" scmap can also be used to project cells from a new experiment onto an annotated reference."}
