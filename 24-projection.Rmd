...
```{r, echo=FALSE, merged_seurat.width = '80%', fig.cap="scmap can be used to compare the annotations of two different samples by providing a one to one mapping between the cells."}
knitr::include_graphics("figures/scmap1.png")
```

```{r, echo=FALSE, merged_seurat.width = '80%', fig.cap=" scmap can also be used to project cells from a new experiment onto an annotated reference."}
...
knitr::include_graphics("figures/cross-dataset-norm.png")
```
## Datasets
...
seger_assigned <- rowSums(t2[c("unclassified endocrine"), 1:9])
```
