...
```{r}
expr_mat <- M3Drop::M3DropConvertData(deng)
```
...
```{r, include=FALSE}
nrow(counts(deng)) - nrow(expr_mat)
summary( rowSums(counts(deng))[! rownames(counts(deng)) %in% rownames(expr_mat) ] )
```
...
DropFS <- NBumiFeatureSelectionCombinedDrop(DANB_fit, method="fdr", qval.thresh=0.01)
...
```{r, eval=FALSE}
cor_feat <- M3Drop::corFS(expr_mat)
