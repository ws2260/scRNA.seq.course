...
Today we will be performing our analysis using a single cell from an mESC dataset produced by (Kolodziejczyk et al. 2015). The cells were sequenced using the SMART-seq2 library preparation protocol and the reads are paired end. Explain where the cell is stored and where FastQC is installed.
...
If you haven't done so already, generate the FastQC report using the command below:

```{bash, eval=FALSE, echo = TRUE}
./FastQC/fastqc -o fastqc_results ERR522959_1.fastq.gz ERR522959_2.fastq.gz
```
...
You can use the command below to trim the Nextera sequencing adapters:

```{bash, eval=FALSE}
./trim_galore --nextera -o fastqc_trimmed_results ERR522959_1.fastq.gz raw_data/ERR522959_2.fastq.gz
```
