...
assay(umi.qc, "ruvg10") <- ruvg$normalizedCounts
assay(umi.qc, "ruvg10_logcpm") <- log2(
    t(t(ruvg$normalizedCounts) / colSums(ruvg$normalizedCounts) * 1e6) + 1
)
```
...
assay(umi.qc, "ruvs10") <- ruvs$normalizedCounts
assay(umi.qc, "ruvs10_logcpm") <- log2(
    t(t(ruvs$normalizedCounts) / colSums(ruvs$normalizedCounts) * 1e6) + 1
)
```
...
```{r}
# Basic batch removal
mod0 = model.matrix(~ 1, data = as.data.frame(t(counts(umi.qc)))) 
# Preserve biological variability
mod1 = model.matrix(~ umi.qc$individual, data = as.data.frame(t(counts(umi.qc)))) 
# adjust for total genes detected
mod2 = model.matrix(~ umi.qc$total_features, data = as.data.frame(t(counts(umi.qc))) 
...
do_glm <- function(data.qc) {
   effects <- apply(exprs(data.qc), 1, glm_fun, batch = data.qc$batch)
   corrected <- exprs(umi.qc) - t(effects[as.numeric(factor(umi.qc$batch)), ])
   return(corrected);
}
...
plotPCA(
    ...
    size_by = "total_features",
    shape_by = "individual",
    exprs_values = "ruvg10_logcpm") +
    ggtitle("PCA - RUVg normalisation: k = 10")
plotPCA(
    umi.qc[endog_genes, ],
    colour_by = "batch",
    size_by = "total_features",
...
plotPCA(
    ...
    ggtitle("PCA - RUVs normalisation: k = 10")
plotPCA(
    umi.qc[endog_genes, ],
    colour_by = "batch",
    size_by = "total_features",
...
boxplot(
    list(
        ...
        "RUVg (k = 2)" = calc_cell_RLE(assayData(umi.qc)$ruvg10_logcpm, erccs),
        "RUVs (k = 1)" = calc_cell_RLE(assayData(umi.qc)$ruvs1_logcpm, erccs),
        "RUVs (k = 2)" = calc_cell_RLE(assayData(umi.qc)$ruvs10_logcpm, erccs),
