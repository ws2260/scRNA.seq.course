...
assay(umi.qc, "ruvg1") <- ruvg$normalizedCounts
assay(umi.qc, "ruvg1_logcpm") <- log2(
    t(t(ruvg$normalizedCounts) / colSums(ruvg$normalizedCounts) * 1e6) + 1
)
ruvg <- RUVg(counts(umi.qc), erccs, k = 2)
assay(umi.qc, "ruvg2") <- ruvg$normalizedCounts
assay(umi.qc, "ruvg2_logcpm") <- log2(
    t(t(ruvg$normalizedCounts) / colSums(ruvg$normalizedCounts) * 1e6) + 1
)
```
...
assay(umi.qc, "ruvs1") <- ruvs$normalizedCounts
assay(umi.qc, "ruvs1_logcpm") <- log2(
    t(t(ruvs$normalizedCounts) / colSums(ruvs$normalizedCounts) * 1e6) + 1
)
ruvs <- RUVs(counts(umi.qc), cIdx, k = 2, scIdx = scIdx, isLog = FALSE)
assay(umi.qc, "ruvs2") <- ruvs$normalizedCounts
assay(umi.qc, "ruvs2_logcpm") <- log2(
    t(t(ruvs$normalizedCounts) / colSums(ruvs$normalizedCounts) * 1e6) + 1
)
```

## Combat
If you have an experiment with a balanced design you Combat can be used to eliminate batch effects while preserving biological effects by specifying the biological effects using the "mod" parameter. However the Tung data contains multiple experimental replicates rather than a balanced design so using "mod1" to preserve biological variability will result in an error. 
```{r}
mod0 = model.matrix(~ 1, data = as.data.frame(t(counts(umi.qc)))) # Basic batch removal
mod1 = model.matrix(~ umi.qc$individual, data = as.data.frame(t(counts(umi.qc)))) # Preserve biological variability
mod2 = model.matrix(~ umi.qc$total_features, data = as.data.fram(t(counts(umi.qc))) # adjust for total genes detected
...
do_mnn <- function(data.qc) {
    ...
    if (dim(batch2)[2] > 0) {
    x = mnnCorrect(
      exprs(batch1), exprs(batch2), exprs(batch3),  
      k = 20,
      sigma = 0.1,
      cos.norm = TRUE,
      svd.dim = 2
...
effects <- apply(exprs(umi.qc), 1, glm_fun, batch = umi.qc$batch, indi = umi.qc$individual)
...
plotPCA(
    ...
    shape_by = "individual",
    exprs_values = "combat") +
    ggtitle("PCA - ComBat batch correction")
plotPCA(
    umi.qc[endog_genes, ],
    colour_by = "batch",
    size_by = "total_features",
    shape_by = "individual",
    exprs_values = "mnn") +
    ggtitle("PCA - mnnCorrect batch correction")
plotPCA(
    umi.qc[endog_genes, ],
    colour_by = "batch",
    size_by = "total_features",
    shape_by = "individual",
    exprs_values = "glm") +
    ggtitle("PCA - glm batch correction")
```
...
boxplot(
    list(
        ...
        "Log2CPM" = calc_cell_RLE(exprs(umi.qc), erccs)
        "RUVg (k = 1)" = calc_cell_RLE(assayData(umi.qc)$ruvg1_logcpm, erccs),
        "RUVg (k = 2)" = calc_cell_RLE(assayData(umi.qc)$ruvg2_logcpm, erccs),
        "RUVs (k = 1)" = calc_cell_RLE(assayData(umi.qc)$ruvs1_logcpm, erccs),
        "RUVs (k = 2)" = calc_cell_RLE(assayData(umi.qc)$ruvs2_logcpm, erccs),
        "ComBat" = calc_cell_RLE(assayData(umi.qc)$combat, erccs),
        "mnnCorrect" = calc_cell_RLE(assayData(umi.qc)$mnn, erccs),
        "glm" = calc_cell_RLE(assayData(umi.qc)$glm, erccs),
    )
)
```
...
```{r confound-cpm, fig.cap = "Explanatory variables (log2CPM)"}
scater::plotQC(
    umi.qc[endog_genes, ],
    type = "expl",
    exprs_values = "exprs",
