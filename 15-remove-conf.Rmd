...
## Combat
```{r}
mod0 = model.matrix(~ 1, data = logcounts(umi.qc))
mod1 = model.matrix(~ umi.qc$individual, data = logcounts(umi.qc))
assay(umi.qc, "combat") <- ComBat(
    dat = logcounts(umi.qc), 
...
## mnnCorrect 
`mnnCorrect` assumes that each batch shares at least one biological condition with each other batch. Thus we merge a replicate from each individual to form three batches. 
```{r}
